...
try:
    while True :
        ...
        while True :
            ...
            while j < 72 :
                if(State_[j] == 1 and State_[j+1] > 0) :
                    Distances_sum += 1/State_[j+1]
                j += 2

            if (done) :
                r = -20
            else :
                r = sign * (1 - 0.1*Distances_sum - 0.2 * sign * abs(math.sin(math.radians(State_[73]*10))) - 0.5 * State_[78]) * State_[72]
